...
def load(agent):
    ...
    #     torch.load('dir_chk/weights_actor_lr00008.pth', map_location=torch.device('cpu')))
    # agent.critic.load_state_dict(
    #     torch.load('dir_chk/weights_critic_lr00008.pth', map_location=torch.device('cpu')))
    agent.policy.load_state_dict(
        torch.load('dir_chk/easy/weights_lr00008_rbx_jupyter_actor_final.pth', map_location=torch.device('cpu')))
    agent.critic.load_state_dict(
        torch.load('dir_chk/easy/weights_lr00008_rbx_jupyter_critic_final.pth', map_location=torch.device('cpu')))
